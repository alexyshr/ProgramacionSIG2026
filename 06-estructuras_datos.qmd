...
# Agregamos Cartagena al final de nuestra ruta (pasa a ser la tercera parada, índice 2)
ruta.append((10.3910, -75.4794))
elevaciones.append(2.0)
print("Ruta con Cartagena al final:", ruta)

# b. Insertar en una posición específica (.insert)
# Necesitamos agregar una parada (ej. Manizales) justo DESPUÉS de Medellín.
# Como Medellín es el índice 1, insertamos en el índice 2.
# Esto empuja a Cartagena hacia el índice 3
...
# la tercera posición de la lista (Base 0), respetando el orden de las paradas anteriores.
...
# Índice 1 (entra a Medellín) y luego Índice 0 (extrae la latitud)
# Nota: A diferencia de R, en Python NO se usan dobles corchetes [[ ]].
# Aquí simplemente encadenamos un corchete al lado del otro: [1][0]
...
# Agregamos la elevación al final del vector
elevaciones <- c(elevaciones, 2.0)

print("Ruta con Cartagena al final:")
print(ruta)

# b. Insertar en una posición específica (after)
# Necesitamos agregar Manizales justo DESPUÉS de Medellín.
# Como Medellín es la posición 2, indicamos after = 2.
# Esto coloca a Manizales en la posición 3 y desplaza a Cartagena a la 4.
...
# NOTA PEDAGÓGICA:
# En R, la función append() es muy versátil. Si no usas 'after', agrega al final.
# Al usar after = 2, le indicamos que el nuevo "eslabón" se enganche justo 
# después de la segunda posición, manteniendo la lógica de nuestra ruta.

# 3. ACCEDER A ELEMENTOS (Índice Base 1)

# LA REGLA DE ORO DE LAS LISTAS EN R:
# ruta[1]   -> Devuelve una LISTA (La "caja" cerrada). No podemos hacer cálculos numéricos con esto.
...
# ACCEDER A LA LATITUD DE LA SEGUNDA CIUDAD DE LA RUTA (MEDELLÍN)
...
# ruta[[2]][1]    -> Toma el primer valor del vector extraído

sda_parada_lat_v2 <- ruta[[2]][1]
cat("Camino Corto - Latitud segunda parada:", sda_parada_lat_v2, "\n")

# NOTA PEDAGÓGICA: 
# ruta[2][[1]] es exactamente idéntico a ruta[[2]]. 
# Usar la primera opción es como abrir un sobre para sacar una carta 
# que está dentro de otro sobre. ¡Doble trabajo innecesario!
...
# múltiples variables en una sola línea. La forma más limpia y "R-ónica" 
...
# b. Insertar en una posición específica (insert!)
# Queremos a Manizales justo DESPUÉS de Medellín. 
# Como Medellín es la posición 2, insertamos en la posición 3.
# Esto desplaza a Cartagena a la posición 4.
...
# el objeto original (mutabilidad). Al ser Base 1, la posición 3 es 
# exactamente el lugar que sigue a Medellín.

# 3. ACCEDER A ELEMENTOS (Índice Base 1)
# Acceder a la primera ciudad de la ruta (Índice 1)
primera_parada = ruta[1]
println("Primera parada: ", primera_parada)

# Acceder a la latitud de la segunda ciudad de la ruta (Medellín)
# Índice 2 (entra a Medellín) y luego Índice 1 (extrae la latitud)
# Nota: A diferencia de R, aquí no hay cajas fuertes [[ ]]. 
...
### Diagrama Conceptual: La Metáfora de la Caja y el Paquete

Para entender cómo cada lenguaje navega por estas estructuras complejas, vamos a plantearnos un objetivo práctico: **extraer la latitud de la segunda parada de nuestra ruta (Medellín)**.

En el siguiente diagrama observaremos el flujo completo, desde cómo se declara la lista de coordenadas, hasta los comandos exactos que debemos introducir en la terminal para llegar al número `6.2442`. 

La clave al leer el gráfico está en notar la diferencia de filosofías: observe cómo Python y Julia le entregan el dato listo para usar o desempaquetar, mientras que R le pone una trampa. Si usa corchetes simples (`[ ]`), R le entregará una "caja fuerte" cerrada que provocará un error matemático si intenta operarla; para obtener el dato real, deberá usar la llave doble (`[[ ]]`) que abre el paquete.
...
%%| fig-cap: "El flujo completo: Desde la declaración de la estructura hasta el desempaquetado de sus elementos."
%%| fig-width: 8
%%| fig-align: center
%%| out-width: "100%"
graph TD
    ...
    %% 2. TÍTULOS
    Python["&nbsp;&nbsp;&nbsp;🐍 PYTHON [Base 0]&nbsp;&nbsp;&nbsp;"]:::tit
    Rlang["&nbsp;&nbsp;&nbsp;🔵 R [Base 1]&nbsp;&nbsp;&nbsp;"]:::tit
    Julia["&nbsp;&nbsp;&nbsp;🟣 JULIA [Base 1]&nbsp;&nbsp;&nbsp;"]:::tit

    %% 3. DEFINICIÓN DE LA VARIABLE INICIAL (Padding masivo de 10 espacios)
    ...
    R_OutS --> R_Box["📦 CAJA CERRADA<br/>[Lista con vector]"]:::caja
    
    R_CmdE --> R_OutE["&nbsp;Terminal: [1] 6.2442 -75.5812&nbsp;"]:::terminal
    R_OutE -->|"Abre la caja"| R_Vec["🔓 PAQUETE ABIERTO<br/>(Vector)"]:::abierto
    
    R_Box -->|"[[1]]"| R_Vec
    R_Box -.->|"Error Matemático"| R_Fail{"❌ No operable"}:::err
...
|:---|:---:|:---:|:---:|
| **Creación** | `[x, y, z]` | `list(x, y, z)` o `c(x, y, z)` | `[x, y, z]` |
...
2. **¿Necesitarás agregar, borrar o intercalar datos después de crearlos?**
   * **SÍ (Mutables):** Usa una **Lista** (o Vector `c()` en R). *Ideal para acumular resultados al final de un proceso (`append`) o intercalar nuevas paradas en medio de una ruta (`insert`).*
...
| Perfil de elevación de una ruta | **Lista / Vector** | Mantiene el orden estricto de los datos y permite mutabilidad posicional. Permite usar funciones matemáticas rápidas (`max`, `mean`). |
...
: Escenarios prácticos para la selección de estructuras de datos {#tbl-seleccion_estructuras tbl-colwidths="[25,20,55]"}

::: {.callout-warning icon="false"}
### ⚠️ La Trampa de R (Recordatorio)
Si en tu análisis decidiste usar una **Lista** en R (`list()`) para agrupar geometrías complejas, nunca olvides la regla de oro de la extracción: usar un corchete simple `[i]` te devuelve una caja cerrada con la que no puedes hacer matemáticas. Para extraer el dato real y operar con él, **siempre debes abrir la caja con el doble corchete `[[i]]`**.
:::
...
| **Insertar en posición *i***| `lista.insert(i, x)` | `append(v, list(x), after=i)`| `insert!(array, i, x)` |
...
4. **El laberinto de los corchetes en R:** Cuando trabajes con listas complejas (`list()`) en R, recuerda la metáfora de la caja fuerte. Un corchete simple `[i]` te devuelve una sub-lista (la caja cerrada), mientras que el doble corchete `[[i]]` te entrega el vector operable (el paquete abierto listo para usarse). ¡Confundirlos es una causa muy común de errores matemáticos!
:::
