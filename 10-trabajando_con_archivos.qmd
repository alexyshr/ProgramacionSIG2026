...
En este ejercicio, vamos a crear un archivo de texto plano (`coordenadas_colombia.txt`) que contendrá las coordenadas (Latitud, Longitud) de cinco de las principales ciudades de Colombia: Bogotá, Medellín, Cali, Barranquilla y Cartagena. Esto nos servirá como base para leer datos tabulares más adelante.
...
f_txt = "coordenadas_colombia.txt"
...
f_in = "coordenadas_colombia.txt"
f_out = "coordenadas_formato.txt"

try:
    # 2. Abrimos el archivo de entrada en modo lectura ('r' de read)
    with open(f_in, "r") as con_in:
...
procesar_archivo_espacial("coordenadas_colombia.txt")
```
:::
:::
...
procesar_archivo_espacial("coordenadas_colombiar.txt")
```
:::
:::
...
procesar_archivo_espacial("coordenadas_colombiaj.txt")
```
:::
:::
...
f_csv = "ciudades_colombia.csv"

# Bloque try-except para crear el archivo CSV
try:
    with open(f_csv, "w") as con_csv:
...
def leer_coordenadas_ciudades(nombre_archivo):
    ...
    try:
        ...
        with open(nombre_archivo, "r") as con_csv:
...
¡Felicidades! Has completado uno de los capítulos más prácticos y fundamentales para tu carrera como analista espacial. Al dominar la lectura de archivos y el manejo de excepciones, has pasado de trabajar con "datos perfectos" a poder enfrentarte a la realidad del geoprocesamiento: bases de datos sucias, archivos incompletos y errores de formato.
