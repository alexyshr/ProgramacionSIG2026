...
### 1.1 Interpolación y formateo de cadenas {#sec-especificadores-formato}

La interpolación nos permite insertar variables directamente dentro de un texto sin tener que estar abriendo y cerrando comillas constantemente. Además, es la herramienta principal para controlar la cantidad de decimales que mostramos en pantalla.

Aunque hoy en día lenguajes como Python y Julia tienen herramientas modernas y muy limpias para hacer esto (`f""` y `$`), la gran mayoría de los lenguajes heredaron un estándar universal creado hace décadas por el lenguaje C. Comprender este estándar es vital, no solo porque es la forma nativa de hacerlo en R, sino porque te permitirá leer código heredado en casi cualquier otro lenguaje.

#### Los especificadores universales (estilo C)

Este estándar utiliza códigos especiales que comienzan con el símbolo de porcentaje (`%`) para reservar el espacio donde irá una variable y decirle al intérprete qué tipo de dato debe esperar:

| Código | Significado | Tipo de Dato Esperado | Ejemplo de uso |
|:---:|:---|:---|:---|
| **`%s`** | **S**tring | Cadenas de texto | Nombres de ciudades, IDs de parcelas. |
| **`%d`** | **D**ecimal / Integer | Números enteros | Conteo de población, años, vértices. |
| **`%f`** | **F**loat | Números decimales | Coordenadas, elevaciones, áreas. |

: Especificadores de formato estándar heredados de C {#tbl-especificadores-formato tbl-colwidths="[15,25,25,35]"}

#### Control de precisión decimal (el uso de `.4f`)

El caso de uso más importante en geomática es el control de los decimales. Las coordenadas suelen tener una precisión altísima en la memoria (ej. `10.836482`), pero en un reporte quizás solo necesitamos 4 decimales.

Para lograr esto, modificamos el especificador `%f` agregándole un punto y el número de decimales deseados: **`.4f`**. Esto le indica al computador que debe mostrar el número redondeándolo exactamente a 4 cifras, rellenando con ceros si es necesario. En Python, esta misma lógica se aplica dentro de las llaves de las f-strings usando dos puntos (`:.4f`).



Veamos cómo conviven los métodos modernos y los clásicos en los tres lenguajes:
...
# 1. F-strings (Interpolación directa y moderna)
print(f"El {pico} tiene una altitud de {altura} msnm.")

# Control de decimales con F-strings (Ej. 4 decimales)
print(f"Latitud exacta: {latitud:.4f}")

# 2. Método antiguo (Estilo C con operador %)
# Es muy común encontrarlo en código heredado o librerías antiguas
# %s para texto, %d para enteros, %f para decimales
print("El %s tiene una altitud de %d msnm y latitud %.4f." % (pico, altura, latitud))
```
:::
:::
...
# En R usamos sprintf de forma nativa como estándar principal
# %s para texto, %d para enteros, %f para decimales
mensaje <- sprintf("El %s tiene una altitud de %d msnm.", pico, altura)
cat(mensaje, "\n")

# Control de decimales obligando a 4 cifras (%.4f)
cat(sprintf("Latitud exacta: %.4f\n", latitud))

# Todo en una sola línea combinada
cat(sprintf("El %s está en la latitud %.4f\n", pico, latitud))
```
:::
:::
...
# 1. Julia usa el símbolo $ para interpolar variables (Recomendado)
println("El $pico tiene una altitud de $altura msnm.")

# 2. Método clásico y control de decimales
# Requiere cargar la librería estándar Printf para usar @sprintf
using Printf

# Controlando la latitud a 4 decimales
lat_formateada = @sprintf("%.4f", latitud)
println("Latitud exacta: $lat_formateada")

# Usando el estilo C completo
# %s para texto, %d para enteros, %f para decimales
println(@sprintf("El %s está en la latitud %.4f", pico, latitud))
```
:::
:::
...
| **Marcadores (Formato)** | `%s`, `%d`, `%f`<br>*(Método clásico)* | `%s`, `%d`, `%f` | `%s`, `%d`, `%f`<br>*(Req. `Printf`)* |
| **Precisión (4 dec)** | `f"{v:.4f}"` | `sprintf("%.4f", v)` | `@sprintf("%.4f", v)` |

: Operaciones principales con cadenas de texto {#tbl-operaciones_strings tbl-colwidths="[28,24,24,24]"}
...
| **Control Decimal (4 dec)** | `f"{lon:.4f}"` | `sprintf("%.4f", lon)` | `@sprintf("%.4f", lon)` |
