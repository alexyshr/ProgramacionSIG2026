  ...
  for (p in partes) {
    ...
    if (is_code) {
      ...
      if (out_triple == 0 && out_comentario <= 0 && p_ev != "") {
        ...
        if (out_comillas == 0) {
          # FIX: Separamos el conteo de bloques for/if para evitar falsos positivos en listas de comprensión
...
j_eval <- function(cmd) {
  ...
  for (l in lineas) {
    ...
    if (en_triple_comilla == 0 && en_comentario_multi <= 0 && l_clean != "") {
      ...
      if (en_comillas == 0) {
        # FIX: Misma lógica aquí para omitir loops for/if empaquetados en listas
        patron_abrir <- "\\b(do|function|begin|let|while|macro|try|module|baremodule|struct|quote)\\b"
        patron_for_if <- "(^|[=;,\\[\\(\\{]|\\b(?:begin|else|return)\\b)\\s*\\b(for|if)\\b"
...
j_plot <- function(cmd, n = "tmp_plot.png", dpi = 300, w = 800, h = NULL, ratio = 1.6, fontsize = 12) {
  ...
  for (l in lineas) {
    ...
    if (en_triple_comilla == 0 && en_comentario_multi <= 0 && l_clean != "") {
      ...
      if (en_comillas == 0) {
        # FIX: Misma lógica aquí para el renderer de gráficas
        patron_abrir <- "\\b(do|function|begin|let|while|macro|try|module|baremodule|struct|quote)\\b"
        patron_for_if <- "(^|[=;,\\[\\(\\{]|\\b(?:begin|else|return)\\b)\\s*\\b(for|if)\\b"
