...
| **Automatización R** | `echo 'Sys.setenv(CHROMOTE_CHROME = "/usr/bin/google-chrome")' >> /usr/local/lib/R/etc/Rprofile.site` | Configura la ruta de Chrome para todo el sistema R de forma persistente. |
...
### 🧪 Batería de Pruebas de Integridad del Docker (Health Checks)
...
| 10 | `docker exec contenedor_sig_unal R -e "library(httpgd); print('Visor OK')"` | Verifica que el motor gráfico para VS Code está listo en el puerto `8787`. |
...
| **Visor R (httpgd)** | `8788` | `8787` | Gráficos de R y VS Code | `http://localhost:8788` |
...
### Lógica de Arquitectura (Dockerfile y Compose) {#sec-arquitectura-puertos}
